...
class EvolutionFieldErrorTest extends AnyFlatSpec with Matchers with TestUtils with BeforeAndAfterEach {
  ...
  // A Java annotation is invisible to the derivation, and the Scala compilers accept it on a field although it targets
  // types only: the constructor parameter carrying it is the only trace left to check
  it should "throw when @version is on a case class field" in {
    Declare.declare[WrongVersionOnField]
